...
describe('Mckinsey receiving invoice', { tags: "mckinsey" }, function () {
    ...
    const projectData = {
        ...
        project_location: {
            "address": "17F Platinum No. 233 Tai Cang Road, Shanghai, 200020, People Republic of China",
            "country": "China",
            "name": "McKinsey & Consulting Company Inc., Shanghai"
        },
    ...
    it('should check that accounting can create the bundle and send the invoice when project location is China and default entity to be United States', function () {
        ...
        invoicePage.getSelectInvoicingEntity().click().type("McKinsey & Consulting Company Inc., Shanghai")
        cy.wait(500)
        ...
        cy.wait('@postCAPICreatePlatformInvoice').its('response').then(postInvoiceResponse => {
            ...
            invoicePage.getInvoicingEntity().should('contain.text', projectDetails.mckinseyDefaultInvoicingEntity)
            ...
            invoicePage.getInvoiceComments().should('have.text', projectDetails.mckinseyChinaInvoiceComment)
            ...
            cy.getMckinseyInvoice(projectData).then(webhookNotification => {
                ...
                expect(parsedInvoiceResponse.invoice_header.invoice_amount).to.not.eq(0)
                ...
                expect(parsedInvoiceResponse.invoice_lines[0].remark).to.eq(projectDetails.mckinseyChinaInvoiceComment)
            })
        })
    })
})

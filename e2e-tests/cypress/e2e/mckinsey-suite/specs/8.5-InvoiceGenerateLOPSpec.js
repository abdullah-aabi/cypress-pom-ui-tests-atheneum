    ...
    it('should check that accounting can create the bundle and generate the invoice details', function () {
        ...
        cy.wait('@postFeeCall').its('response').then(postFeeResponse => {
            ...
            cy.wait('@postCAPICreatePlatformInvoice').its('response').then(postInvoiceResponse => {
                ...
                feePage.getFeeFormNotificationMessages().last().should('contain.text', projectDetails.feeLOPProjectTypeMessage)
                ...
                globalPage.getStatusLabelByPosition(3).should('contain.text', 'ENS API calls only')
                globalPage.getStatusLabelByPosition(4).should('contain.text', 'ENS Free calls invoice')
            })
        })
    })
})

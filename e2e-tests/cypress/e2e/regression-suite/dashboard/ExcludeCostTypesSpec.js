...
describe('Verify Team Revenue  details', { tags: "regression" }, function () {
    let testUsers, authToken, testData, projectDetails, costsTestData,
        count, grossMargin, teamName, accountManagerName, projectId,
        dv, iv, revenue, hono, target, achievement, resultGrossMargin, resultTarget,
    ...
    it('Verify Team Revenue widget for whole team', function () {
            ...
            .then(getTeamRevenueWidgetResponse => {
                ...
                if (count > 0) {
                    dashboardPage.getTeamMembersCountInTeamRevenue()
                        ...
                        .should('eq', count)
                    // Get Team Name
                    dashboardPage.getTeamNameInTeamRevenue()
                        .should('be.visible')
                        .then($el => {
                            teamName = $el.text()
                            expect(teamName).to.include(getTeamRevenueWidgetResponse.body.teamName)
                            expect(teamName).to.include(testData.dashboardTeamName)
                        })
                    // Account Manager name
                    dashboardPage.getAccountManagernameInTeamRevenue()
                        .should('be.visible')
                        .then($el => {
                            accountManagerName = $el.text().trim()
                            expect(accountManagerName).to.include(getTeamRevenueWidgetResponse.body.accountManager)
                    ...
                    // Name of employee
                    dashboardPage.getColumnInTeamRevenue(1, 1)
                        .should('be.visible')
                        .then($empName => {
                    ...
                    dashboardPage.getColumnInTeamRevenue(2, 9)
                        ...
                        .then($empAchievement => {
                            ...
                            expect(parseInt(grossMarginForASBefore)).to
                                .eq(0)
                            expect(parseInt(achievement)).to
                                .eq(parseInt((grossMarginForASBefore / parseInt(target)) * 100))
                        })
                }
            })
        // Create test data 
        cy.createProjectFromAPI(projectName, 'Expert Sessions', testUsers.dashboardAccountManager.emailAddress, testData.dashboardOfficeName).then(
            projectCreateResponse => {
                ...
                cy.verifyNotificationAndClose()
                expertPipelinePage.getDelieveredByField().scrollIntoView().click()
                    .clear().type(`${testUsers.dashboardTeamLeader.firstName} ${testUsers.dashboardTeamLeader.lastName}`)
                ...
                expertInvitePage.getExpertsPipelineButton().scrollIntoView().click()
                cy.changeEPLStatus(expertData.originalName, 'Interviewed')
                scheduling.getConfirmInterviewButton().click()
                scheduling.getCurrencyTypeFieldList().first().should('have.text', projectDetails.feeCurrency)
                scheduling.getInterviewDurationField().should('have.value', projectDetails.interviewDuration)
                scheduling.getFeeValueAmountOnBundle().should('have.value', projectDetails.feeAmountField)
                ...
                // Added free call and Internal cost to check if its not included in dashboard
                scheduling.getFreeCallCheckbox().check()
                scheduling.selectFreeCallCostType('Internal - Free trial')
                scheduling.getConfirmInterviewButton().click()
                globalPage.getNotificationTitle().should('have.text', 'Saved')
                globalPage.getNotificationMessage().first().should('contain.text', 'Hon. & Fee Bundle succeeded.')
                cy.checkEPLStatus(expertData.originalName, 'Interviewed')
                cy.verifyExpertReplyStatus(expertData.originalName, 'Confirmed')

                // Add Referral cost to check if its not included in dashboard
                ...
                projectOutcomePage.selectDeliverableType(costsTestData.deliverableType)
                projectOutcomePage.selectCostType(costsTestData.costTypeToExclude)
                projectOutcomePage.selectExpert(expertData.originalName)
                projectOutcomePage.selectCurrentDate()
                projectOutcomePage.selectCostCurrencyType(costsTestData.currencyType)
                projectOutcomePage.getCostValueInput().type(costsTestData.costValue)
                projectOutcomePage.selectDeliveredBy(`${testUsers.dashboardTeamLeader.firstName} ${testUsers.dashboardTeamLeader.lastName}`)
                projectOutcomePage.getSaveCostButton().click()
                globalPage.getNotificationTitle().should('have.text', 'Saved')
                globalPage.getNotificationMessage().should('contain.text', costsTestData.successfulMessage)
                ...
                if (count > 0) {
                    ...
                    dashboardPage.getColumnInTeamRevenue(1, 5)
                        ...
                        .then($empRevenue => {
                            ...
                            totalRevenueForTL = revenueForTLBefore + parseInt(projectDetails.feeAmountField)
                    ...
                    dashboardPage.getColumnInTeamRevenue(1, 6)
                        ...
                        .then($empHono => {
                            ...
                            totalHonoForTL = honoForTLBefore + parseInt(projectDetails.honorariumAmount)
                    ...
                    dashboardPage.getColumnInTeamRevenue(1, 9)
                        ...
                        .then($empAchievement => {
                            ...
                            achievement = achievement.split(',').join('')
                            expect(parseInt(achievement)).to
                                .eq(parseInt((grossMarginForTLAfter / totalTargetForTL) * 100))
                        })
                    ...
                    dashboardPage.getColumnInTeamRevenue(2, 9)
                        ...
                        .then($empAchievement => {
                            ...
                            achievement = achievement.split(',').join('')
                            expect(parseInt(achievement)).to
                                .eq(parseInt((totalgrossMarginForAS / totalTargetForAS) * 100))
    ...
    it('Verfiy Team Revenue widget for subteams', function () {
        ...
        dashboardPage.getClolumnInSubTeam(1, 9)
            ...
            .then($empAchievement => {
                ...
                achievement = achievement.split(',').join('')
                expect(parseInt(achievement)).to
                    .eq(parseInt((grossMargin / target) * 100))
            })
        // Name of employee

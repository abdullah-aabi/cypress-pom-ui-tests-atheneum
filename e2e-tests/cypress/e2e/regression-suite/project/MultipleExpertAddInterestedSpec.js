...
describe('Associate searching for experts and add multiple experts blocked by Sherlock to a project', { tags: "specs_with_issues" }, function () {
    ...
    it('should Add as Interested multiple experts to a project and add a Sherlock comment for all', function () {
        expertSearchPage
            .getExpertNameField()
        ...
        cy.requestSearchExperts(authToken, expertSearchRequestBody).then(
            expertSearchResult => {
                ...
                cy.clickInviteActionButton('As interested')
                ...
                projectDetailsPage.getEPLStatus().each(expertStatus =>
                    expect(expertStatus.text().trim()).to.equal('Interested')
                )
            }
        )
    })
